...
import { PATH_SPORT, PATH_GAMES, PATH_WEB_PUSH } from '#app/constants/paths';
...
describe('Main.js - public', () => {
  let mockModules = {};
  const waitForAllPromisesToResolve = async () =>
  ...
  it('will correctly initialise all base modules that should load each page on execution', async () => {
    ...
    expect(mockModules.loadRegisterWall).not.toHaveBeenCalled();
  ...
  it('will only load register wall module on register wall supported page', async () => {
    mockModules.loadRegisterWall.mockResolvedValueOnce(null);
    jest
      .spyOn(document, 'getElementById')
      .mockReturnValueOnce('some-reg-wall-element-wrapper');
    await executeMainJs();
    expect(mockModules.loadRegisterWall).toHaveBeenCalledTimes(1);
  ...
  describe('init Braze in app message with delay module', () => {
    ...
    it('will not load module on non-es non-live blog articles', async () => {
      ...
      mockModules.isImageLedArticleOnNewsSection.mockReturnValueOnce(false);
      await executeMainJs();
      expect(mockModules.initBrazeInAppMessageWithDelay).not.toHaveBeenCalled();
    });

    it('will load module on non-es image led articles on news section or sub section', async () => {
      global.JSGlobals = {
        domain: PUBLICATION_UK,
        isLiveBlog: false,
      };
      mockModules.isImageLedArticleOnNewsSection.mockReturnValueOnce(true);
      await executeMainJs();
      expect(mockModules.initBrazeInAppMessageWithDelay).toHaveBeenCalledTimes(
        1,
      );
      expect(mockModules.isImageLedArticleOnNewsSection).toHaveBeenCalledTimes(
        1,
      );
    });
  });
  ...
  describe('betting module', () => {
    const paths = [
      { name: 'exactly matches sport', path: PATH_SPORT },
      { name: 'exactly matches games', path: PATH_GAMES },
      { name: 'includes sport', path: `${PATH_SPORT}/some-other-path` },
      { name: 'includes games', path: `${PATH_GAMES}/some-other-path` },
    ];
    ...
    it.each(paths)(
      'should load if the domain is UK and the path $name',
      async ({ path }) => {
    ...
    it('should not load if the path does not include sport or games', async () => {
      window.location.pathname = '/some-other-path';
      await executeMainJs();
      expect(mockModules.bettingAnalyticsTag).not.toHaveBeenCalled();
    });

    it('should not load if the domain is not UK', async () => {
      global.JSGlobals.domain = 'some-non-uk-domain';
      window.location.pathname = PATH_SPORT;
      await executeMainJs();
      expect(mockModules.bettingAnalyticsTag).not.toHaveBeenCalled();
    });
  });
