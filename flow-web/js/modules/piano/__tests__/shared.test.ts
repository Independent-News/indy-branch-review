...
import hasHomepageRefreshed from '#app/public/js/utils/pageRefresh/hasHomepageRefreshed';
...
import * as thatModule from '../shared';
...
jest.mock('#app/public/js/utils/pageRefresh/hasHomepageRefreshed');
jest.mock('../../util');
...
describe('Piano shared public js', () => {
  ...
  describe('appendTinypassScript()', () => {
    it('will correctly load the Piano SDK script', async () => {
  ...
  describe('initPianoSDKIfHomepageHasNotRefreshed()', () => {
    const mockHasHomepageRefreshed =
      hasHomepageRefreshed as jest.MockedFunction<typeof hasHomepageRefreshed>;
    let appendTinyPassScriptSpy: jest.SpyInstance;

    beforeEach(() => {
      appendTinyPassScriptSpy = jest
        .spyOn(thatModule, 'appendTinypassScript')
        .mockImplementation();
    });
    ...
    it('will not load the Piano SDK on a page refreshed view', async () => {
      mockHasHomepageRefreshed.mockReturnValueOnce(true);
      await initPianoSDKIfHomepageHasNotRefreshed();
      expect(appendTinyPassScriptSpy).not.toHaveBeenCalled();
      expect(mockHasHomepageRefreshed).toHaveBeenCalledOnce();
    });

    it('will load the Piano SDK on a page view which is not refreshed', async () => {
      mockHasHomepageRefreshed.mockReturnValueOnce(false);
      await initPianoSDKIfHomepageHasNotRefreshed();
      expect(appendTinyPassScriptSpy).toHaveBeenCalledOnce();
    });
  });
});
